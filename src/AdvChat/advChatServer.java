...
public class advChatServer {
    ...
    //Users name and writers
    private static HashMap<String, PrintWriter> hashmap = new HashMap<>();
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (var listener = new ServerSocket(8080)) {
            while (true) {
                pool.execute(new advChatServer.Handler(listener.accept()));
            }
        }
    }
    ...
    private static class Handler implements Runnable {
        ...
        public void run() {
            try {
                ...
                while (true) {
                    out.println("Input your chat handle!");
                    name = in.nextLine();
                    if (name == null) {
                        return;
                    }
                ...
                while (true) {
                    ...
                    if (input.toLowerCase().startsWith("/quit")) {
                        ...
                        sendPrivatMessage(input);
                    }else if(input.toLowerCase().startsWith("/list")){
                        getListOfAllUsers();
                    }else{
                        writeToAllUsers(input);
                    }
        ...
        private void writeToAllUsers(String input) {
            for (PrintWriter writer : writers) {
        ...
        private void sendPrivatMessage(String input) {
            String[] test = input.split(" ");
            PrintWriter receiver = hashmap.get(test[1]);
            String[] newArray = Arrays.copyOfRange(test, 2, test.length);
            // String message = Arrays.toString(newArray);
            String message = String.join(" ", newArray);

            System.out.println("Privat besked er kommet fra " + name + ": " + test[1]);
            receiver.println("MESSAGE From " + name + ": To " + test[1]+ ": " + message);
        }

        private void getListOfAllUsers() {
